...
for params in ParameterGrid(param_grid):
    print(f"Training SVM with parameters: {params}")
    
    #Create the pipeline
    pipeline = Pipeline([
        ('scaler', StandardScaler()),
        ('pca', PCA(n_components=50)), #reduce to 50 components for speed
        ('svc', SVC(kernel=params['svc_kernel'], C=params['svc_C'], gamma=params.get('svc_gamma', 'scale'), degree=params.get('svc_degree', 3), probability=True))
    ])
